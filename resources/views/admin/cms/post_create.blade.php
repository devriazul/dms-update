            ...
            <form action="" method="post" id="createPostForm" enctype="multipart/form-data">
                ...
                <div class="form-group row {{ $errors->has('post_content')? 'has-error':'' }}">
                    <div tabindex="-1" class="col-sm-12">
                        <textarea tabindex="-1" name="post_content" id="post_content" class="form-control" rows="6">{{ old('post_content') }}</textarea>
                        {!! $errors->has('post_content')? '<p class="help-block">'.$errors->first('post_content').'</p>':'' !!}
                    </div>
                </div>
                <div class="form-group row">
                    <div class="col-sm-12">
                        <input type="text" class="form-control" id="tags" value="{{ old('tags') }}" name="tags" placeholder="Tags" data-role="tagsinput">
                    </div>
                </div> 
                <div class="form-group row">
                    <div class="col-sm-12">
                        <textarea placeholder="Meta Description" name="meta_description" id="meta_description" class="form-control" rows="3">{{ old('meta_description') }}</textarea>
